...
addon = xbmcaddon.Addon('plugin.video.ontouch.tv')

profile = xbmc.translatePath(addon.getAddonInfo('profile'))
__settings__ = xbmcaddon.Addon(id=re.compile("plugin://(.+?)/").findall(sys.argv[0])[0])
home = __settings__.getAddonInfo('path')
favorites = os.path.join( profile, 'favorites' )
REV = xbmc.translatePath( os.path.join( profile, 'list_revision') )
icon ="http://www.123srilanka.net/livetv/icon.png"
fanart = "www.123srilanka.net/livetv/fanart.jpg"
source_file="http://www.123srilanka.net/livetv/vod.xml"
if sys.argv[2]=="?mod=sub":
    source_file="http://www.123srilanka.net/livetv/sub/vod.xml"
if sys.argv[2]=="?mod=ond":
    source_file="http://www.123srilanka.net/livetv/ond/vod.xml"
if os.path.exists(favorites)==True:
    FAV = open(favorites).read()

f = urllib.urlopen(source_file)
SOURCES = f.read()
f.close()      
...
def addSource(new="false"):
        ...
        xbmc.executebuiltin("XBMC.Notification(Ontouch TV,New source added.,5000,"+icon+")")
...
def getUpdate():
        req = urllib2.Request('http://download.ontouch.tv/addons/')
        ...
        for i in files:
            name = i('a')[0]['href']
            url = 'http://download.ontouch.tv/addons/'+name
        ...
        xbmc.executebuiltin("XBMC.Notification(Ontouch TV,Community Lists Updated,5000,"+icon+")")


def checkForUpdate():
        url = 'http://download.ontouch.tv/addons//'
        req = urllib2.Request(url)
        response = urllib2.urlopen(req)
        link=response.read()
        response.close()
...
if __settings__.getSetting('community_list') == "true":
    if __settings__.getSetting('save_location') == "":
        xbmc.executebuiltin("XBMC.Notification('Ontouch TV','Choose a location to save files and select OK to save.',15000,"+icon+")")
...
def getSoup(url):
        ...
        if url.startswith('http://'):
            try:
                ...
                if hasattr(e, 'code'):
                    print 'We failed with error code - %s.' % e.code
                    xbmc.executebuiltin("XBMC.Notification(Ontouch TV,We failed with error code - "+str(e.code)+",10000,"+icon+")")
                elif hasattr(e, 'reason'):
                    print 'We failed to reach a server.'
                    print 'Reason: ', e.reason
                    xbmc.executebuiltin("XBMC.Notification(Ontouch TV,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")
        else:
...
def addLink(url,name,iconimage,fanart,description,genre,date,showcontext=True):
        ...
        if showcontext:
            try:
                if name in FAV:
                    contextMenu = [('Remove from Ontouch TV Favorites','XBMC.Container.Update(%s?mode=6&name=%s&url=%s&iconimage=%s)' %(sys.argv[0], urllib.quote_plus(name), urllib.quote_plus(url), urllib.quote_plus(iconimage)))]
                else:
                    contextMenu = [('Add to Ontouch TV Favorites','XBMC.Container.Update(%s?mode=5&name=%s&url=%s&iconimage=%s)' %(sys.argv[0], urllib.quote_plus(name), urllib.quote_plus(url), urllib.quote_plus(iconimage)))]
            except:
                contextMenu = [('Add to Ontouch TV Favorites','XBMC.Container.Update(%s?mode=5&name=%s&url=%s&iconimage=%s)' %(sys.argv[0], urllib.quote_plus(name), urllib.quote_plus(url), urllib.quote_plus(iconimage)))]
